...
TODO (lab44): Env-specific ConfigMaps/Secrets (`SPRING_DATASOURCE_*` / Lab 42 `CRM_DB_*`, Ingress host) stay **outside**
the JAR. Secret **names** only in Git. User is **`crm`**, not `crm_app`. No Kafka requirement for this lab’s smoke.

## DB compatibility

TODO (lab44): Expand-before-contract. Isolated DB **`crm_lab43`** — do not treat Lab 42’s `crm_lab42` as production.
Record when digest rollback cannot undo a DROP COLUMN.