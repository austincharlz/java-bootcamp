...
`DefaultErrorHandler` retries with bounded backoff (`1s`, `2` retries), then publishes to DLT.
Non-retryable contract failures:
...
- **Publish-after-success (this lab):** simple and direct; if DB commit succeeds but Kafka publish fails, downstream consumers miss the event.
- **Transactional outbox (production):** persist an outbox row in the same DB transaction, then relay to Kafka for stronger delivery guarantees.